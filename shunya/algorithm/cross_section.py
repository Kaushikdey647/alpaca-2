...
def winsorize(x: jnp.ndarray, tail: float) -> jnp.ndarray:
    ...
    Clips finite values to ``[quantile(tail), quantile(1 - tail)]`` using **only**
    finite inputs for quantiles. Non-finite coordinates become ``0``.

    ``tail`` in ``[0, 0.5)``; ``tail == 0`` returns ``x`` with non-finite → ``0``.
    """
    ...
    if tail == 0:
        finite = jnp.isfinite(x)
        return jnp.where(finite, x, 0.0).astype(jnp.float32)

    arr = np.asarray(x, dtype=np.float64)
    finite_m = np.isfinite(arr)
    if not finite_m.any():
        return jnp.zeros_like(x, dtype=jnp.float32)
    lo = float(np.quantile(arr[finite_m], tail))
    hi = float(np.quantile(arr[finite_m], 1.0 - tail))
    finite = jnp.isfinite(x)
    clipped = jnp.clip(x, lo, hi)
    return jnp.where(finite, clipped, 0.0).astype(jnp.float32)


@jax.jit
def neutralize_market(s: jnp.ndarray) -> jnp.ndarray:
    """
    Subtract cross-sectional mean over **finite** scores; non-finite → ``0``.

    Dollar-neutral weights after L1 scaling sum to ~0 when multiple finite names exist.
    """
    s = jnp.asarray(s, dtype=jnp.float32)
    finite = jnp.isfinite(s)
    s_f = jnp.where(finite, s, 0.0)
    cnt = jnp.sum(finite.astype(jnp.float32))
    mu = jnp.where(cnt > 0, jnp.sum(s_f) / jnp.maximum(cnt, 1.0), 0.0)
    return jnp.where(finite, s - mu, 0.0).astype(jnp.float32)


@partial(jax.jit, static_argnames=("num_segments",))
def _neutralize_groups_finite_jit(
    s: jnp.ndarray, inv: jnp.ndarray, num_segments: int
) -> jnp.ndarray:
    inv = jnp.asarray(inv, dtype=jnp.int32)
    finite = jnp.isfinite(s)
    s_f = jnp.where(finite, s, 0.0)
    ones_f = finite.astype(jnp.float32)
    counts = segment_sum(ones_f, inv, num_segments=num_segments)
    gsum = segment_sum(s_f, inv, num_segments=num_segments)
    means = jnp.where(counts > _EPS, gsum / jnp.maximum(counts, _EPS), 0.0)
    return jnp.where(finite, s - means[inv], 0.0).astype(jnp.float32)
...
def neutralize_groups(s: jnp.ndarray, group_ids: jnp.ndarray) -> jnp.ndarray:
    ...
    return _neutralize_groups_finite_jit(
        s_j, jnp.asarray(inv, jnp.int32), num_segments
    )
...
def rank(x: jnp.ndarray) -> jnp.ndarray:
    ...
    **smallest** raw value → **0**, **largest** → **1** (ties: ``argsort`` order).
    ...
    Non-finite values sort last (largest rank index); all non-finite map to ``1.0``
    when ``n > 1``.
    ...
    x = jnp.asarray(x, dtype=jnp.float32)
    n = x.shape[0]
    safe = jnp.where(jnp.isfinite(x), x, jnp.inf)
